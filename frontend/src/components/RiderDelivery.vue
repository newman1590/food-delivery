<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-actions>
            ...
            <v-dialog v-model="deliveryDiagram" width="500">
                <DeliveryCommand
                    @closeDialog="closeDelivery"
                    @delivery="delivery"
                ></DeliveryCommand>
            </v-dialog>
        </v-card-actions>
	...
	async created() {
        ...
        methods: {
            ...
            async delivery(params) {
                try {
                    if(!this.offline) {
                        var temp = await axios.put(axios.fixUrl(this.value._links['delivery'].href), params)
                        for(var k in temp.data) {
                            this.value[k]=temp.data[k];
                        }
                    }
