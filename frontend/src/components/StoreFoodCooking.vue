<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-actions>
            ...
            <v-dialog v-model="startDiagram" width="500">
                <StartCommand
                    @closeDialog="closeStart"
                    @start="start"
                ></StartCommand>
            </v-dialog>
            <v-btn
            ...
            <v-dialog v-model="finishDiagram" width="500">
                <FinishCommand
                    @closeDialog="closeFinish"
                    @finish="finish"
                ></FinishCommand>
            </v-dialog>
        </v-card-actions>
	...
	async created() {
        ...
        methods: {
            ...
            async start(params) {
                try {
                    if(!this.offline) {
                        var temp = await axios.put(axios.fixUrl(this.value._links['start'].href), params)
                        for(var k in temp.data) {
                            this.value[k]=temp.data[k];
                        }
                    }
            ...
            openStart() {
                this.startDiagram = true;
            },
            closeStart() {
                this.startDiagram = false;
            },
            async finish(params) {
                try {
                    if(!this.offline) {
                        var temp = await axios.put(axios.fixUrl(this.value._links['finish'].href), params)
                        for(var k in temp.data) {
                            this.value[k]=temp.data[k];
                        }
                    }
            ...
            openFinish() {
                this.finishDiagram = true;
            },
            closeFinish() {
                this.finishDiagram = false;
            },
        },
    }
</script>
